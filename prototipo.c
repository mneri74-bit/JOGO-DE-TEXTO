...
static const char *palavras_dicio[] = {
    "abobora", "acontecimento", "aeronave", "arquitetura", "astronauta",
    "borboleta", "civilizacao", "computador", "conhecimento", "constelacao",
    "descoberta", "eletricidade", "engenharia", "experiencia", "felicidade",
    "gravidade", "imaginacao", "independencia", "labirintico", "matematica"
};
...
void sorteio_palavras(int onda, char palavras[][50]) {
    ...
    // Limite de segurança para não travar o jogo se a onda for maior que o dicionário
    int quantidade = onda;
    if (quantidade > NUM_PALAVRAS) {
        quantidade = NUM_PALAVRAS;
    }
...
if (onda > NUM_PALAVRAS) {
    quantidade = NUM_PALAVRAS; // Trava no limite do dicionário
} else {
    quantidade = onda; // Usa o número da onda normalmente
}
    ...
    for(int i = 0; i < quantidade; i++) {
        ...
        do {
            colisao = 0;
            y[i] = -(rand() % 10); // Começam fora da tela, em cima
...
int main() {
    ...
    while(vidas > 0) {
        ...
        int quantidade = (onda > NUM_PALAVRAS) ? NUM_PALAVRAS : onda;
            ...
            if ((tempo - timer) > intervalo(onda)) {
                ...
                for (controle = 0; controle < quantidade; controle++) {
                    ...
                    if (coordY[controle] < info.dwSize.Y - 3) {
                        ...
                        coordY[controle]++; // Move para baixo

                        // Desenha a palavra na nova posição
                        ...
                        // A palavra bateu no chão
                        vidas--;
                        strcpy(palavras[controle], ""); // Destrói a palavra
                        if (alvo == controle) { alvo = -1; letra = 0; } // Solta o alvo
